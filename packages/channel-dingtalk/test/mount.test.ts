/**
 * channel-dingtalk transactional-mount rollback test (doc §5 / R2).
 *
...
describe('channel-dingtalk transactional mount', () => {
  it('start() rejection rolls back: registry empty, signal aborted, stop() called', async () => {
    const ctx = new Context();
    new ChannelService(ctx);

    let capturedSignal: AbortSignal | undefined;
    const stop = vi.spyOn(DingTalkAdapter.prototype, 'stop').mockImplementation(async function () {
      return undefined;
    });
    vi.spyOn(DingTalkAdapter.prototype, 'start').mockImplementation(
      async function (this: DingTalkAdapter, adapterCtx: ChannelAdapterContext) {
