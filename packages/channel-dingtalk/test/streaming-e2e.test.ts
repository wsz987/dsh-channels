/**
 * M2 acceptance: DingTalk AI Card streaming driven by the generic ReplyRouter.
 *
...
function makeAdapter(transport: FakeTransport): DingTalkAdapter {
  return new DingTalkAdapter(makeConfig(), { transport, now: () => Date.now() });
}
...
function captureHandle(
  adapter: DingTalkAdapter,
): { handle: () => DingTalkCardReply | undefined } {
  const captured = { handle: undefined as DingTalkCardReply | undefined };
...
describe('M2 acceptance: AI Card streaming through the generic ReplyRouter', () => {
  it('throttles card updates below chunk count and finalizes with full text (throttle/finalize)', async () => {
    ...
    // Acceptance (e): the streamed reply used createReply (edit), never send.
  ...
  it('deduplicates identical inbound payloads before forwarding (dedup)', async () => {
    ...
    expect(received[0]?.conversation.id).toBe('conv_1');
