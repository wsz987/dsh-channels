/**
 * Weixin auth state machine with optional JSON-file persistence.
 *
...
import type { PollAuthResult, WeixinAuthStatus, WeixinUpstream } from './upstream.js';

export interface WeixinAuthState {
  /** `unknown` is an internal pre-load state; the upstream never reports it. */
  status: WeixinAuthStatus | 'unknown';
  userId?: string;
  updatedAt: number;
}

export interface WeixinAuthManagerOptions {
  upstream: WeixinUpstream;
  /** Optional file path for persistence. */
  statePath?: string;
  /** Injectable clock (tests). */
  now?: () => number;
  onAuthChange: (state: WeixinAuthState) => void;
}

/** Auth statuses exposed to the channel contract. */
export const toChannelAuthState = (status: WeixinAuthStatus): AuthStatePoll['state'] =>
  status === 'authenticated' ? 'authenticated' : status === 'pending' ? 'pending' : status === 'expired' ? 'expired' : 'failed';

export class WeixinAuthManager {
  private status: WeixinAuthStatus | 'unknown' = 'unknown';
  private userId?: string;
  private readonly now: () => number;

  constructor(private readonly options: WeixinAuthManagerOptions) {
    this.now = options.now ?? Date.now;
  }

  getState(): WeixinAuthState {
  ...
  async beginAuth(): Promise<AuthChallenge> {
    ...
    return {
      id: `wx-auth-${this.now()}`,
      instruction: 'scan the QR code with WeChat to authenticate',
      qrUrl: payload.qrUrl,
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      const parsed = JSON.parse(text) as Partial<WeixinAuthState>;
