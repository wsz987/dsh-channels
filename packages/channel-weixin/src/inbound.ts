...
import { dedupKey, mapInbound, type WeixinInboundMeta } from './mapper.js';
...
export class InboundProcessor {
  ...
  /** msgId -> last-seen timestamp, pruned on every handle. */
  ...
  async handle(raw: unknown): Promise<void> {
    ...
    if (this.options.dedupEnabled) {
      ...
      if (last !== undefined && now - last < this.options.dedupWindowMs) {
        this.options.ctx.logger.debug(`[channel-weixin] dropped duplicate message '${key}'`);
        return;
      }
