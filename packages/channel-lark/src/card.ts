/**
 * Lark editable card — the streaming reply (`ReplyHandle`).
 *
 * Lark streaming is modeled as: create card → update card with content
...
export class LarkCardReply implements ReplyHandle {
  ...
  private record(kind: LarkCardUpdate['kind'], text?: string, error?: unknown): void {
    this.updates.push({
      kind,
      text,
      at: this.now(),
